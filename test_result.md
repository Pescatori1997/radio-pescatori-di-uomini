...
## --- TRAGUARDI DEL CAMMINO (Achievements / Bacheca) — Fase 1 ---
backend:
  - task: "Achievements: /me/achievements auto-unlock (soglie da metriche reali), storico immutabile in user_achievements; admin CRUD medaglie + order + assign/unassign; walk-board settings; seed 12 medaglie default"
    implemented: true
    working: "NA"
    file: "backend/server.py (blocco Traguardi del Cammino ~riga 1536), backend/achievements_seed.py"
    priority: "high"
    needs_retesting: true
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "Seed idempotente (seed_key) di 12 medaglie Bronzo/Argento/Oro su 4 metriche: plans (piani completati: 1/3/7), podcasts (history: 1/10/30), meditations (like+pray+comment distinti: 1/10/25), verses (bookmarks: 1/15/50) + walk_board default. GET /api/me/achievements calcola counts reali via _user_metric_counts, sblocca automaticamente (earned) e persiste earned_at in user_achievements con $setOnInsert (storico mantenuto anche se admin cambia soglie). Admin (require_perm 'achievements'): GET /admin/achievements, GET/POST(201)/PATCH/DELETE /admin/achievements/{id}, POST /admin/achievements/order, POST /admin/achievements/{id}/assign|unassign (per email), GET/PATCH /admin/walk-board. Verificato via curl: 12 medaglie seedate, utente demo con 3 piani/12 podcast/2 versetti/1 meditazione -> earned=6 (bronze+silver dove soglia raggiunta), progress e count corretti."
frontend:
  - task: "Bacheca del Cammino: armadio in legno con ante che si aprono (reduce-motion), medaglie appese a ganci per categoria, overlay medaglia con flip 3D fronte/retro; card Home + voce Profilo; Admin CRUD + impostazioni bacheca"
    implemented: true
    working: "NA"
    file: "frontend/app/traguardi.tsx, frontend/src/components/bacheca/{BachecaDelCammino,Medal,MedalDetailOverlay,wood}.ts(x), frontend/src/components/BachecaCard.tsx, frontend/app/(tabs)/index.tsx, frontend/app/(tabs)/profilo.tsx, frontend/app/admin/achievements/{index,[id]}.tsx, frontend/src/components/AdminShell.tsx"
    priority: "high"
    needs_retesting: true
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "Screenshot-verificato su web: /traguardi mostra armadio in legno (noce), header principio 'NON È UNA GARA. È UN CAMMINO.' + intro + badge '6 traguardi raggiunti su 12'; 4 mensole per categoria (Piani Biblici/Podcast/Meditazioni/Versetti) con targhette incise, rail in ottone e medaglie appese; earned a colori (bronzo/argento/oro) con logo emblema, locked in grigio con lucchetto + progresso (3/7, 12/30, 1/10, 1/25); ante si aprono in animazione all'apertura. Tap medaglia -> overlay in primo piano con backdrop, flip 3D: fronte (categoria, medaglia, titolo, tier), retro (statistica grande '3', 'Piani completati', descrizione, 'Ottenuto il 4 agosto 2026'). Card Home 'home-bacheca' e voce Profilo 'Traguardi del Cammino' presenti. Admin: sidebar 'Traguardi del Cammino' -> /admin/achievements (impostazioni bacheca collassabili + lista medaglie con reorder frecce, toggle attivo, duplica, elimina) + editor [id] (tier/metric/threshold/emoji/descrizione/immagine/attivo + assegna/revoca per email)."
agent_communication:
    -agent: "main"
