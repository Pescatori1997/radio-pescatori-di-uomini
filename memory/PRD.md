...
## Fix Diretta streaming URL + consolidamento menu (2026-06/08)
- **BUG risolto**: incollando un URL di diretta (YouTube/Facebook/Vimeo/Twitch) il video NON partiva perché `diretta.tsx` lo passava a `expo-video` (che riproduce solo file diretti MP4/HLS, non le pagine dei provider).
- **FIX**: nuovo player embed `src/components/live/EmbedFrame.tsx` (WebView nativo) + `.web.tsx` (iframe). In `diretta.tsx`, se il media on-air (o il filler) è un provider riconosciuto (`detectProvider`), si costruisce l'URL embed con autoplay+mute (`liveEmbedSrc` in `utils/embeds.ts`) e si mostra in EmbedFrame invece di expo-video; per HLS .m3u8/MP4/GridFS/audio resta expo-video/expo-audio. Verificato: on-air con URL YouTube → iframe `youtube.com/embed/<id>?...autoplay=1&mute=1` renderizzato correttamente.
- Editor programma: hint aggiornato (YouTube/Facebook/Vimeo/Twitch link diretto oppure HLS .m3u8).
- **Consolidamento menu admin** (richiesta utente): rimosse dalla sidebar (AdminShell NAV) le voci `radio` (Radio), `control` (Centro di Controllo Radio), `streaming` (Gestione Live). Tutta la gestione dirette/video ora avviene dal **Palinsesto** (programmi + URL diretta in tempo reale per-programma + Riempitivo). Le route/endpoint sottostanti restano (radio player pubblico intatto), solo non più esposte nel menu.

## Avviso diretta gestibile + Auto-salvataggio registrazioni (2026-06/08)
- **Avviso pre-diretta gestibile dall'admin**: in Admin > Palinsesto > "Impostazioni Diretta" nuovi controlli: toggle "Avvisa i fedeli prima della diretta" (live_notif_enabled) + "Minuti di anticipo dell'avviso" (live_notif_minutes, 1–180, default 10). Lo scheduler `_send_live_prenotifications` ora legge questi valori da settings (prima era hardcoded LIVE_NOTIF_MINUTES=10); se disabilitato non invia.
- **Auto-salvataggio registrazioni**: toggle "Salva automaticamente le registrazioni" (autosave_recordings, default ON). Nuovo scheduler `_autosave_recordings` (stesso loop, ogni 60s): al termine dello slot di un programma con `broadcast_media_url`, aggiunge automaticamente una puntata (`episodes` $push) al programma stesso con audio_url=URL diretta (per i live YouTube l'URL diventa la replica/VOD), titolo "<programma> · <data>", data odierna, durata calcolata dallo slot. Idempotente per programma/giorno (collezione `live_recorded`); salta se esiste già una puntata con stessa URL+data. Le puntate appaiono in "Le puntate" della pagina programma (tasto "AVVIA ULTIMA PUNTATA" + lista, apre l'URL).
- Backend: GeneralSettings + `live_notif_enabled/live_notif_minutes/autosave_recordings`. Frontend: admin/schedule/index.tsx (ASwitch + AInput). Verificato: autosave crea 1 puntata (durata 85 min) ed è idempotente; UI admin mostra i 3 controlli.

## Rifiniture Diretta: messaggio avviso, copertina puntate, player interno (2026-06/08)
- **Messaggio avviso personalizzabile**: settings `live_notif_title` + `live_notif_body` (placeholder {titolo} e {minuti}); usati da `_send_live_prenotifications` (fallback al testo automatico se vuoti). Admin > Palinsesto > Impostazioni Diretta: campi Titolo/Testo avviso (visibili se avviso ON) + hint sui placeholder.
...
## Personalizzazione Sito — Fase 3: Testi estesi + Metadati sezione (2026-06)
- **Testi del sito estesi**: aggiunti gruppi a `src/siteTexts.ts` per Podcast (segnaposto ricerca, nessun risultato, "In evidenza"), Notizie (ricerca, vuoto, "Leggi di più"), Palinsesto (vuoto), Merch (etichetta "SHOP UFFICIALE"). `SiteTextGroupKey` allargato a string. Editor `/admin/site-texts.tsx` ora mostra e salva tutti i gruppi.
- **Metadati sezione** (nuova schermata `/admin/section-meta.tsx`): per ogni sezione l'admin modifica Nome, Sottotitolo, Descrizione, Copertina (immagine base64 via AImagePicker) e Visibilità ON/OFF. Metadati salvati in `site_settings.sections[key]` (via PUT /admin/site-settings deep-merge); la visibilità riusa il campo esistente `GeneralSettings.section_visibility` (unica fonte di verità, già consumata da tab/Home/Profilo). Ordine gestito altrove (Layout Home / Navigazione) — indicato nella UI.
- **Catalogo** `src/sectionMeta.ts` (`SECTION_META_CATALOG`): podcast, news, palinsesto, merch, prayer, prayer_board, messages, contact, biblioteca. Ogni voce espone solo i campi realmente wireati allo schermo.
- **Context** esteso: `SiteTextsContext` ora espone anche `sm(key)` → `{name,subtitle,description,image}` (vuoti filtrati → fallback nello schermo). Fetch unico di `/site-settings`.
- **Wiring frontend** (fallback sicuro al testo originale se vuoto): `podcast.tsx` (nome/sottotitolo/descrizione/copertina + ricerca/vuoto/in evidenza), `news.tsx` (nome/sottotitolo/descrizione/copertina + ricerca/vuoto/leggi di più), `palinsesto.tsx` (nome/sottotitolo + vuoto), `merch/index.tsx` (nome/sottotitolo + kicker), `prayer.tsx` (nome/intro), `prayer-board.tsx` (nome/sottotitolo), `contact.tsx` (nome + sottotitolo opzionale), `messages.tsx` (nome/intro), `biblioteca.tsx` (nome fallback a label + sottotitolo).
- Hub `/admin/site.tsx`: attivati i riquadri "Testi del sito" e "Metadati sezione"; resta "Aspetto" come prossima fase.
- **Backend invariato** (usato deep-merge esistente). **Bug trovato e corretto** dal testing agent: `site-texts.tsx save()` inviava solo home+player → ora invia l'intera mappa `texts` (verificato via API).
- Verificato: testing agent 7/7 backend, frontend admin+pubblico OK, fallback OK, nessuna regressione (Home/Player/Notizie/Palinsesto). Report: `/app/test_reports/iteration_49.json`.
- **Prossimo (Fase 4)**: "Aspetto del sito" (colori/stile) — su richiesta dell'utente, DOPO verifica di questa fase.

## Personalizzazione Sito — Fase 4: Aspetto (colore brand/accento) (2026-06)
- Ambito confermato dall'utente: SOLO colore brand/accento + palette predefinite. Nessun tema completo, nessun dark mode, nessuna forma. Sfondi/testi/struttura invariati.
- **Meccanismo (chiave)**: il tema `src/theme.ts` è importato staticamente in 134 file (656 usi di `colors.brand*`), quindi un tema "live" via hook sarebbe un refactor enorme e rischioso. Soluzione a zero-refactor: **custom entry `frontend/index.js`** (package.json `main` → `index.js`) che, PRIMA di caricare `expo-router/entry` (e quindi prima che gli StyleSheet degli schermi vengano creati), legge la palette dalla cache locale (AsyncStorage/localStorage) e **muta in place l'oggetto condiviso `colors`** con gli accenti scelti. Confermato funzionante su web (log + screenshot: seed 'rose' → tutta l'app diventa rosa: hero, badge IN DIRETTA, pulsanti, link, icone). Su native usa lo stesso entry con require differito.
- **File nuovi**: `src/palettes.ts` (7 palette eleganti: sky[default], ocean, teal, sage, royal, amber, rose — ognuna con brandPrimary/Secondary/Tertiary/onBrandTertiary), `src/appearance.ts` (applyPalette che muta colors, cachePaletteKey, loadAndApplyCachedPalette; chiave `pdu_appearance_palette`), `frontend/index.js` (bootstrap entry), `app/admin/appearance.tsx` (picker palette + anteprima + salva).
- **Persistenza**: palette salvata in `site_settings.appearance.palette` (PUT /admin/site-settings, perm 'site'). `SiteTextsContext` al fetch di /site-settings scrive la palette in cache locale → si applica dal launch successivo su tutti i dispositivi. L'admin, al salvataggio: PUT backend + cache locale + mutazione live + su web `window.location.reload()` per effetto immediato.
- **Fallback**: nessuna palette / palette sconosciuta → design di default (sky). Non distruttivo.
...
## Bibbia — Selettore versione + aggiunta Diodati 1641 (2026-06)
- Richiesta utente: rendere cliccabile l'etichetta della versione (era hardcoded "Riveduta (Luzzi 1927)") per scegliere tra le versioni, e aggiungere "Diodati 1641" (pubblico dominio).
- Backend: `bible_seed.py` generalizzato a lista `TRANSLATIONS` (seed per-traduzione idempotente, catalogo `bible_translations` aggiornato a ogni avvio). Aggiunta Diodati: scaricata da getBible v2 (`giovanni.json`, "Giovanni Diodati Bible (1649)", Public Domain, stesso schema del Riveduta) e salvata in `backend/data/diodati_1649.json` (66 libri, 31.102 versetti). Codice interno `diodati_1649`; nome visualizzato "Diodati 1641" (come richiesto dall'utente; il testo digitale pubblico dominio è la revisione 1649 della traduzione Diodati). Endpoint esistenti già multi-traduzione (`translation` param).
- Frontend `app/lettore/index.tsx`: l'etichetta versione è ora una pill cliccabile (icona libro + chevron); modale "Versione della Bibbia" elenca le traduzioni da `/bible/translations` con radio-check; selezione persistita in AsyncStorage `bible_translation`; ricarica i libri nella versione scelta; propaga `&translation=<code>` al lettore e alla "Continua a leggere".
...
## Pulizia doppioni pulsanti (2026-06)
- Richiesta utente: in ogni sezione lasciare UN solo pulsante per azione (evitare doppioni che fanno la stessa cosa).
- Bibbia (`app/lettore/index.tsx`): rimossa l'icona "Piani di Lettura" nell'header (duplicava il grande riquadro "Piani di Lettura" → /lettore/piani). Restano in header solo Salvati + Cerca.
- Home Palinsesto (`app/(tabs)/index.tsx`, scelta utente 1b): rimossi il link "Vedi tutti" del SectionHeader Palinsesto e il pulsante "Visualizza palinsesto"; resta solo la card on-air (toccabile → /palinsesto). `SectionHeader` reso con seeAll/onPress opzionali (invariato per Podcast che mantiene "Vedi tutti").
- Revisione sezione-per-sezione: gli altri casi NON sono veri doppioni → prayer-board "+" (header) vs "Condividi una richiesta" (solo empty-state, non simultanei); prayer/messages 2× /login sono prompt condizionali; podcast/news/merch/donate sono item di lista. Lasciati invariati.

## Fix pagina Programma (app/programma/[slug].tsx) (2026-06)
- Bug 1 "Contattaci → Programma non trovato": con contact_url non valido (es. "test") il vecchio codice faceva Linking.openURL("test") → su web/PWA veniva risolto come URL relativo → navigava a /programma/test → "Programma non trovato". FIX: contact() ora valida — email→mailto, http(s)→apri, altrimenti (vuoto/invalido) apre la pagina in-app /contact.
- Bug 2 "Indietro non funziona": `router.back()` senza history non fa nulla. FIX: `goBack()` = canGoBack ? back : replace("/"). Applicato al pulsante Indietro (stato non trovato) e al back dell'hero.
- Bug 3 "Condividi link che non porta a nulla": condivideva evangelic-stream.emergent.host/programma/slug (redirige alla landing QR). FIX: shareUrl preferisce il link pubblico della diretta/programma se presente (broadcast_media_url esterno / social.youtube / facebook / instagram), altrimenti fallback al link .host. Così il link apre direttamente la live quando il programma ha un URL pubblico.
- Informazioni: la tab mostra long_description/host/giorni/orario/categoria/social dai dati admin (wired, nessun bug). Se l'admin inserisce contenuti reali, appaiono.
- Verifica: bundle pulito; E2E in preview limitato (nessun programma schedulato oggi + gate su deep-link + dati "test" solo in produzione). Da riverificare sull'app pubblicata dopo Publish.

## Diretta no-seek + Palinsesto timeline precisa (2026-06)
- No avanti/indietro nelle dirette: il VideoView nativo aveva già nativeControls={false}. Aggiornato `src/utils/embeds.ts` liveEmbedSrc → per YouTube aggiunge controls=0&disablekb=1&fs=0&modestbranding=1&iv_load_policy=3 (niente barra/seek) e per Vimeo controls=0. Vale per registrati E live: nessuno può spostarsi avanti/indietro; tutti vedono la stessa posizione sincronizzata. (liveEmbedSrc è usato solo nella schermata Diretta.)
- Palinsesto timeline precisa: `app/(tabs)/palinsesto.tsx` renderCard ora accetta offsetMin; il primo programma di ogni fascia oraria riceve marginTop = minuti dopo l'ora (1px/min su righe da 60px), così un programma alle 12:15 si allinea a 15' dopo il marcatore 12:00 invece che a 12:00. Orario esatto già mostrato in cardTime.
- Verifica: bundle pulito; Palinsesto renderizza (in preview nessun programma schedulato oggi, quindi l'offset si vede in produzione con orari reali). No-seek verificabile in diretta reale su build/produzione.

## FIX CRASH Android all'avvio (build 1.0.1) (2026-06)
- Sintomo: APK 1.0.1 installa ma si chiude subito all'apertura (crash on launch). Web/preview OK.
- Causa individuata (root cause): l'entry personalizzato `frontend/index.js` (introdotto con la funzione "Aspetto/palette") registrava il componente root in modo ASINCRONO: `loadAndApplyCachedPalette().finally(() => require("expo-router/entry"))`. Su build nativa Android in RELEASE (newArchEnabled=true) il runtime chiama runApplication("main") subito dopo il caricamento del bundle; se registerRootComponent non è avvenuto in modo sincrono → fatal "Application 'main' has not been registered" → crash immediato. Sul web il timing è tollerante, per questo l'anteprima non crashava.
...
## Implemented (2026-06, session — Piani di Lettura "a libro" / libreria a scaffali)
- Richiesta utente (con mockup): trasformare i piani biblici da scroll infinito a esperienza "a libro" con libreria a scaffali per categoria, copertine tipo libro, segnalibri per giornata e pagina-giornata a pergamena.
- **Libreria a scaffali** (`app/lettore/piani.tsx`, riscritta): header "📖 Piani Biblici" + sottotitolo + toggle "I miei piani / Tutti". Piani raggruppati per `category` (ordine backend); ogni categoria = header con icona (mappata dal nome) + riga orizzontale di libri in piedi su una **mensola di legno** (LinearGradient marrone + lip). "Vedi tutti" quando categoria > 4 → vista `?cat=` a griglia 2 colonne. Badge progresso/✓ sui libri iscritti.
- **Componente `src/components/plans/BookCover.tsx`**: libro 3D (spine highlight + ombra) con copertina immagine (overlay "N GIORNI") o fallback pergamena colorata (titolo serif + N GIORNI, colore variato per titolo). Prop `width/style/enrolled/percent/completed`.
- **Copertina/indice piano** (`app/lettore/piano/[id].tsx`, riscritta): hero glow marino + book cover centrata, titolo, descrizione, meta pill (N Giorni · 5-7 min al giorno · N Segnalibri), sezione "I TUOI SEGNALIBRI" = nastri orizzontali per giornata (completati verdi + ✓, giornata da riprendere in oro), pulsante "CONTINUA/INIZIA IL PIANO" (auto-enroll + apre giornata da riprendere), card "CARATTERISTICHE". Rimosso lo scroll infinito dei giorni espansi.
- **Pagina-giornata** (nuovo `app/lettore/piano/giorno.tsx`, `?id=&day=`): tema pergamena (font serif), nastro GIORNO N, titolo, ornamento pesce, MEDITAZIONE, VERSETTI BIBLICI (testo reale dei versetti fetchato via `bibleChapter` e affettato per verse_start/end; capitolo intero → card "Leggi il capitolo completo"), RIFLESSIONE PERSONALE (TextInput salvato in AsyncStorage per piano+giorno). Footer: sfoglia Precedente/Successiva + "Segna come letto" (toggle completamento). Header con progresso + segnalibro (toggle) + condividi.
...
## Updated (2026-06 — Tempo di lettura configurabile + Piani consigliati)
- **Tempo di lettura giornaliero configurabile da Admin**: nuovo campo `reading_time` (stringa libera, es. "5-7 min", "10 min al giorno") in `ReadingPlanIn` + persistito in create/update + esposto in `_plan_public` e nel dettaglio. Editor admin: campo "Tempo di lettura giornaliero" (`plan-reading-time`). La scheda del piano mostra `plan.reading_time` nella meta pill (fallback "5-7 min al giorno") invece del fisso.
- **Rimossa la sezione "CARATTERISTICHE"** dalla scheda del piano (spiegazione superflua) e **aggiunta "PIANI CONSIGLIATI"**: riga orizzontale di copertine (BookCover) con altri piani pubblicati, stessa categoria per primi, max 8, che aprono il rispettivo piano. Fetch via `api.readingPlans()` nel load del dettaglio.

## Updated (2026-06 — Card di condivisione a libro + messaggio personalizzabile)
- **Messaggio di condivisione personalizzabile da Admin**: nuovo campo `share_message` (stringa libera) in `ReadingPlanIn` + persistito create/update + esposto in `_plan_public` e dettaglio. Editor admin: campo multiline "Messaggio di condivisione" (`plan-share-message`) sotto la descrizione ("se vuoto usa la descrizione"). Serve a creare curiosità senza svelare la descrizione completa.
- **SharePlanSheet ridisegnata (modo piano)**: la copertina è ora resa a **forma di libro** (ritratto 130×174, dorso 3D con highlight+ombra, centrata) invece del banner landscape; titolo/sottotitolo/messaggio centrati; il testo usa `plan.share_message || plan.description`. Card più alta (ratio 1.5 in modo piano, captureSize 1080×1620) → **fix sovrapposizione** footer/contenuto. Il testo condiviso (WhatsApp/Telegram/…) usa anch'esso `share_message`.
- Verificato via screenshot: libro verticale + messaggio personalizzato ("Dio non ha finito di scrivere la tua storia…") + badge giorni + footer senza overlap.

## Implemented (2026-06 — Piani Biblici: creazione manuale estesa + IMPORT da ZIP)
- **Doppia modalità di creazione** (il "+" apre `/admin/reading-plans/create` con due scelte, sistema manuale NON rimosso):
  - ✍️ **Crea manualmente** → editor esistente `/admin/reading-plans/new`.
  - 📦 **Importa piano completo** → `/admin/reading-plans/import`.
- **Nuovo campo giorno `talk`** (🙏 "Parla con il Signore"): aggiunto a `PlanDay` (backend), all'editor manuale (TextInput per giorno) e alla pagina-giornata pubblica. `PlanReading.book_nr` reso opzionale (per riferimenti non risolti).
- **Pagina-giornata** riordinata in struttura editoriale: **📖 LETTURA BIBLICA DEL GIORNO** (testo versetti reale) → **🙏 PARLA CON IL SIGNORE** (talk) → **💭 MEDITAZIONE** → RIFLESSIONE PERSONALE.
- **Import ZIP**: endpoint `POST /admin/reading-plans/parse-import` (body `{zip_b64}`) — legge lo ZIP (piano.json + immagine copertina), valida e restituisce un body pronto (copertina come data URI, letture strutturate) SENZA salvare. Frontend `import.tsx`: DocumentPicker (web=FileReader, native=FileSystem legacy base64) → anteprima (copertina, titolo, descrizione, categoria, N giorni, elenco giorni) → **Annulla / Importa come bozza / Importa e pubblica** (riusa `POST /admin/reading-plans` con lo status scelto). Piani importati identici ai manuali (modificabili/eliminabili).
  - JSON supportato: `title, description, category, duration_days, cover_image, status, days[]` con `day, title, bible_reading[] (stringhe), talk_with_the_lord, meditation`. Anche `subtitle/reading_time/share_message` opzionali.
  - Parsing riferimenti `_parse_bible_ref` via BOOKS map + alias IT (Salmo→Salmi, Cantico, ecc.); label originale mantenuta. Riferimenti non risolti restano come testo (non tappabili).
  - Validazione con messaggi chiari IT: JSON valido, title/description/category/duration/cover presenti, copertina presente nello ZIP, n° giorni == duration_days, nessun giorno duplicato/mancante (1..N), ogni giorno con titolo+meditazione+≥1 lettura. Nessuna creazione parziale (400 con dettaglio).
- Verificato: backend E2E via requests (parse OK, Salmo→Salmi, errori validazione 400 chiari, create); pagina-giornata pubblica con le 3 sezioni editoriali + talk (screenshot). Admin UI dietro gate Google (coperta a livello API).

## Updated (2026-06 — Categorie piani predefinite)
- L'editor piani ora mostra **categorie predefinite selezionabili** (chip) al posto del solo testo libero: `src/utils/planCategories.ts` (Crescita Spirituale, Vita Cristiana, Famiglia, Dipendenze, Attualità, Emozioni e Mente, Relazioni, Preghiera, Fede e Dubbi, Speranza, Nuovi Credenti, Tematici) + chip "+ Personalizzata" che rivela il campo testo per una categoria custom. In modifica, se la categoria salvata non è tra i preset, parte in modalità personalizzata. Le categorie guidano gli scaffali della libreria.
- Nota copertine: nella panoramica a scaffali la foto del piano viene già mostrata quando impostata (BookCover); il segnaposto compare solo se il piano non ha copertina.
